import { IAppState, DeviceType } from '@/ADempiere/modules/app/AppType'
import { recursiveTreeSearch } from '@/ADempiere/shared/utils/valueUtils'
import { INotificationProcessData } from '@/ADempiere/modules/process'
...
export default class MixinProcessActivity extends Vue {
  ...
  get getterAllInExecution(): any[] {
    return this.$store.getters[Namespaces.Process + '/' + 'getAllInExecution']
  }

  // process local and send with response
  get getterAllFinishProcess(): INotificationProcessData[] {
    return this.$store.getters[Namespaces.Process + '/' + 'getAllFinishProcess']
  }
  ...
  get getRunProcessAll(): any[] {
    ...
    processAll.forEach(element => {
      ...
      let infoMetadata:
        | Partial<IProcessData>
        | undefined = this.getProcessMetadata(element.processUuid)
      if (!infoMetadata) {
        infoMetadata = {}
      }
      ...
      const indexRepeat = processAllReturned.findIndex(
        item =>
          item.instanceUuid === element.instanceUuid && element.instanceUuid
      )
      if (indexRepeat > -1) {
  ...
  getProcessMetadata(uuid: string): IProcessData | undefined {
    return this.$store.getters[
      Namespaces.ProcessDefinition + '/' + 'getProcess'
    ](uuid)
  }
  ...
  beforeMount() {
    this.$store
      .dispatch(
        Namespaces.Process + '/' + 'getSessionProcessFromServer',
        {
